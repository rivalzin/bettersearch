...
public abstract class OptionRowsScreen extends Screen {
    ...
    private boolean beginScrollbarDrag(double mouseX, double mouseY, int button) {
        if (button != com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT || !overScrollbar(mouseX, mouseY)) {
            return false;
        }
    ...
    private void renderPanel(GuiGraphicsExtractor guiGraphics) {
        ...
        Component title = panelRow != null ? panelRow.title : panelDefaultTitle();
        Component description = panelRow != null ? panelRow.description : panelDefaultDescription();
        Identifier preview = panelRow != null ? panelRow.preview : null;
