...
public final class BetterSearchClient {
        ...
        CompletableFuture
                ...
                .whenComplete((table, error) -> minecraft.execute(() -> {
                    ...
                    if (table == null || table.isEmpty()) {
                        BetterSearch.LOGGER.warn("[{}] the direct read came back empty -"
    ...
    // the slow half of a first search is reading the languages, so do it on the tick instead.
    // After the first success this costs two boolean reads.
    public static void warmUp() {
        if (!isEnabled() || Minecraft.getInstance() == null
