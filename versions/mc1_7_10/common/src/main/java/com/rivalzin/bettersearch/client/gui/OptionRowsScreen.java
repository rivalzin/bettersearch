...
public abstract class OptionRowsScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        panelWidth = MathHelper.clamp_int((this.width - 3 * MARGIN) * 36 / 100, 110, 224);
    ...
    private void measurePreview() {
        ...
        for (Row row : rows) {
            ...
            int lines = this.fontRendererObj.listFormattedStringToWidth(row.title, textWidth).size()
                    + this.fontRendererObj.listFormattedStringToWidth(row.description, textWidth).size();
    ...
    private void layoutRows() {
        ...
        scrollRow = MathHelper.clamp_int(scrollRow, 0, maxScroll);
        for (int i = 0; i < rows.size(); i++) {
            ...
            row.control.yPosition = row.y + (ROW_HEIGHT - row.control.height) / 2;
            if (row.reset != null) {
                row.reset.visible = shown;
                row.reset.yPosition = row.y + (ROW_HEIGHT - RESET_SIZE) / 2;
            }
        }
    }
    ...
    private boolean updateScrollbarDrag(int mouseY) {
        ...
        if (max > 0 && usable > 0) {
            ...
            scrollRow = MathHelper.clamp_int((int) Math.round(travelled * max / usable), 0, max);
            layoutRows();
        }
        return true;
    }
    ...
    private void renderTip(int mouseX, int mouseY) {
        for (Object raw : this.buttonList) {
            ...
            boolean over = mouseX >= widget.xPosition && mouseY >= widget.yPosition
                    && mouseX < widget.xPosition + widget.width && mouseY < widget.yPosition + widget.height;
            if (!over) {
                continue;
            }
    ...
    private void renderPanel() {
        ...
        for (Object line : this.fontRendererObj.listFormattedStringToWidth(title, textWidth)) {
            this.fontRendererObj.drawString(String.valueOf(line), textX, y, Theme.ACCENT);
            y += 10;
        }
        y += 4;
        for (Object line : this.fontRendererObj.listFormattedStringToWidth(description, textWidth)) {
            if (y + 9 > textLimit) {
                break;
            }
            this.fontRendererObj.drawString(String.valueOf(line), textX, y, Theme.TEXT);
            y += 10;
        }
    }
    ...
    private static void drawScaledTexture(int x, int y, int drawWidth, int drawHeight,
    ...
    private String ellipsize(String text, int maxWidth) {
        ...
        if (this.fontRendererObj.getStringWidth(text) <= limit) {
            return text;
        }
        return this.fontRendererObj.trimStringToWidth(text, limit - this.fontRendererObj.getStringWidth("...")) + "...";
    }
}
